...
JavaScript对象都是**一个属性集合，相互之间没有任何联系。是可以定义对象的类，让每个对象都共享某些属性。**

在JavaScript中，**类的实现是基于原型继承机制。**
...
构造函数是用来初始化新创建的对象。使用new调用构造函数会自动创建一个新对象，因此**构造函数本身只需初始化这个新对象的状态**

构造函数的prototye属性被用做新对象的原型。这意味着通过**同一个构造函数创建的所有对象都继承自一个相同的对象，**因此它们都是**同一个类的成员**

### 构造函数和类的标识

初始化对象的状态的构造函数则不能作为类的标识，两个构造函数的prototype属性可能指向同一个原型对象；

构造函数是类的“外在表现”；构造函数的名字通常用做类名

### constructor属性

构造函数的**原型中存在预先定义好的constructor属性，这意味着对象通常继承的constructor均指代它们的构造函数**

## 类的扩充

对象从其原型继承属性，如果创建对象之后原型的属性发生改变，也会影响到继承这个原型的所有实例对象，可以通过给原型对象添加新方法来扩充JavaScript类。

## 类和类型

### instanceof运算符

左操作数是待检测其类的对象，右操作数是定义类的构造函数；检测对象的原型链上是否存在某个特定的原型对象；可以使用isPrototypeOf()方法

### constructor属性

为构造函数是类的公共标识，所以最直接的方法就是使用constructor属性
